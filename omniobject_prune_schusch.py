...
for scene_i in tqdm.tqdm(sub_scenes_list):
    ...
    prune_output_dir = os.path.join(gaussian_raw_save_path, scene_i ,'point_cloud_prune')
    ...
    prune_percent = 0.66 # start value
    print(f"######## processing {scene_i} ###############", scene_tgt_path, prune_output_dir, point_cloud_path)

    os.system(f"python prune_finetune.py -s {scene_tgt_path} -m {prune_output_dir} --eval --port {port} --start_pointcloud {point_cloud_path} --iteration 35000 --prune_percent {prune_percent} --prune_type v_important_score --prune_decay 1 --position_lr_max_steps 35000")
