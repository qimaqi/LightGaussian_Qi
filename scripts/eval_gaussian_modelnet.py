...
if __name__ == "__main__":
	...
	for cat_id in modelnet_dict.keys():
		for task in modelnet_dict[cat_id].keys():
			for id_num, obj_id in enumerate(modelnet_dict[cat_id][task]):
				obj_id = obj_id.split('.')[0]
				obj_total_path.append(os.path.join(blender_renders_path, cat_id,task, obj_id))
			

	if args.end_idx > len(obj_total_path) or args.end_idx == -1 :
		args.end_idx = len(obj_total_path) 

	save_name = f'modelnet_gaussian_{args.start_idx}_{args.end_idx}.json'
	...
	for obj_path in tqdm(obj_total_path):
		...
		task = obj_path.split('/')[-2]
		cat_id =  obj_path.split('/')[-3]
		if cat_id not in modelnet_dict_gs.keys():
			modelnet_dict_gs[cat_id] = {}
		if task not in modelnet_dict_gs[cat_id].keys():
			modelnet_dict_gs[cat_id][task] = []
		...
		if os.path.exists(pointcloud_gs): #and os.path.exists(num_align_gs):
			...
			modelnet_dict_gs[cat_id][task].append(obj_i_metric_dict)
			# print("shapenet_v1_dict_gs", shapenet_v1_dict_gs)
			count+=1
		else:
			print("Non exist", pointcloud_gs)
		

		if count % 1000 == 0:
			# # save everytime finish one cat
			with open(save_name, 'w') as f:
				json.dump(modelnet_dict_gs, f, indent=4)
	...
	psnr_results = []
	gaussin_num  =[]
	for cat_id in modelnet_dict_gs.keys():
		for obj_i in modelnet_dict_gs[cat_id]:
