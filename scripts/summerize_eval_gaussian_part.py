...
if __name__ == "__main__":
	...
	parser.add_argument('--summerize_file', type=str, default='/cluster/work/cvl/qimaqi/3dv_gaussian/LightGaussian_Qi/scripts/shapnetv1_gaussian_all.json', 
					help='place_saving_dict')
	args = parser.parse_args()

	with open('/cluster/work/cvl/qimaqi/3dv_gaussian/LightGaussian_Qi/shapnet_part.json', 'r') as f:
		shapenet_v1_part= json.load(f)

	part_id_list = []

	for cat_id in shapenet_v1_part.keys():
		for obj_i in shapenet_v1_part[cat_id]:
			part_id_list.append(str(cat_id) + '-' + str(obj_i))
	# example dict
	...
	count = 0
	for obj_i in shapenet_v1_dict:
		if obj_i in part_id_list:
			count+=1
			mmt_results.append(float(shapenet_v1_dict[obj_i]['mmd_xz']))
			psnr_results.append(float(shapenet_v1_dict[obj_i]['psnr']))
			gaussin_num.append(float(shapenet_v1_dict[obj_i]['gaussian_nums']))
			jsd_results.append(float(shapenet_v1_dict[obj_i]['jsd_xz']))
