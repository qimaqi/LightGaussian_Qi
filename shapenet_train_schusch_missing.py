...
missing_txt=np.loadtxt('/cluster/work/cvl/qimaqi/3dv_gaussian/LightGaussian_Qi/scripts/missing.txt', dtype=str)
missing_ids = []
for i in range(len(missing_txt)):
    missing_ids.append(missing_txt[i].split('-')[1].split('.')[0])
...
if  args.end_idx > len(missing_ids) or args.end_idx == -1:
    args.end_idx = len(missing_ids)

print("total length of missing_ids", len(missing_ids))
missing_ids = missing_ids[args.start_idx:args.end_idx]
print("sub_scenes_list", len(missing_ids))

problematic_scenes = []
error_scenes = {}
finsh_scenes = []
# print("missing_ids", missing_ids)
missing_ids = ['65c7a3d8fb95311c492d9da2668ec34c']
for scene_i in tqdm.tqdm(obj_total_path):
    if not scene_i.split('/')[-1] in missing_ids:
        continue

    start_t = time.time()
    # in first time we run without active sampling
    prune_output_dir = os.path.join(scene_i ,'light_gs_demo')
    if args.org_gs:
...
np.savetxt(f'./missing_problematic_scenes.txt', problematic_scenes, fmt='%s')
np.savetxt(f'./missing_finished_scenes.txt', finsh_scenes, fmt='%s')
