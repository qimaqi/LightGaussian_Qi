...
parser.add_argument('--start_idx', type=json.loads, 
                help='start scene you want to train.')
parser.add_argument('--end_idx', type=json.loads,
...
assert len(args.start_idx) == len(args.end_idx), "start_idx and end_idx should have same length"
if len(args.start_idx) == 1:
    start_idx = args.start_idx[0]
    end_idx = args.end_idx[0]
    if  end_idx > len(obj_total_path) or end_idx == -1:
        end_idx = len(obj_total_path)

    sub_objects_list = obj_total_path[start_idx:end_idx]

else:
    # multiple start and end
    sub_objects_list = []
    for start_idx_i, end_idx_i in zip(args.start_idx, args.end_idx):
        if  end_idx_i > len(obj_total_path) or end_idx_i == -1:
            end_idx_i = len(obj_total_path)
        sub_objects_list.extend(obj_total_path[start_idx_i:end_idx_i])
    start_idx = args.start_idx[0]
